    ...
    def test_source_has_no_external_client_or_execution_imports(self):
        ...
        for path in source_root.glob('*.py'):
            ...
            if path.name == 'dependency_runtime.py':
                ...
                self.assertIn('FINAL_EVIDENCE_READBACK_AUDIT_IMPLEMENTATION_COUNT = 1', text)
                self.assertIn('FILESYSTEM_EVIDENCE_READ_IMPLEMENTATION_COUNT = 1', text)
                self.assertIn('def audit_persisted_final_evidence_records(', text)
                self.assertEqual(text.count('def audit_persisted_final_evidence_records('), 1)
                self.assertIn('def _read_persisted_evidence_bytes(', text)
                self.assertEqual(text.count('def _read_persisted_evidence_bytes('), 1)
                self.assertIn('def _read_file_flags(', text)
                self.assertEqual(text.count('def _read_file_flags('), 1)
                ...
                for name in (
                    ...
                    with self.subTest(path=path.name, required_zero=name):
                        self.assertIn(name, text)
            elif path.name == 'cloud_boundary.py':
                for token in dependency_operational_tokens + source_read_tokens + evidence_write_tokens:
                    with self.subTest(path=path.name, token=token):
                        self.assertNotIn(token, text)
                ...
                self.assertEqual(text.count('evidence.build_pre_final_evidence_manifest('), 2)
                self.assertEqual(text.count('evidence.build_final_result('), 2)
                ...
                self.assertEqual(text.count('human_gate.build_human_decision('), 1)
